...
static int show_version (void)
{
  ...
  if (opt.do_version >= 2)
  {
    ...
    const char *clang = compiler_clang_version();
 ...
 */
static directory_array *dir_array_add_or_insert (const char *dir_or_file, bool is_cwd, bool insert_at0)
{
  directory_array *d = CALLOC (1, sizeof(*d));
  struct stat      st;
  int    max, i, exp_ok = (dir_or_file && *dir_or_file != '%');
  bool   exists = false;
  bool   is_dir = false;

  if (!opt.lua_mode && safe_stat(dir_or_file, &st, NULL) == 0)
  {
    is_dir = exists = _S_ISDIR (st.st_mode);
    if (opt.file_mode)
       exists |= _S_ISREG (st.st_mode);
  }

  d->cyg_dir  = NULL;
  d->dir      = STRDUP (dir_or_file);
  d->exp_ok   = exp_ok;
   ...
   */
  d->is_native = (stricmp(dir_or_file, sys_native_dir) == 0);
  ...
  if (d->is_native && !have_sys_native_dir)
     TRACE (2, "Native dir_or_file '%s' does not exist.\n", dir_or_file);
  else if (!d->exist)
  {
  ...
  for (i = 0; i < max-1; i++)
  {
    ...
    if (str_equal(dir_or_file, d2->dir))
       d->num_dup++;
  }
  return (d);
}
...
smartlist_t *split_env_var (const char *env_name, const char *value)
{
  ...
  if (!value)
  {
    TRACE (1, "Calling 'split_env_var(\"%s\", NULL)'!\n", env_name);
    return (NULL);
  }
  ...
  */
  ...
  /* Insert CWD at position 0 if not done it already
   */
 ...
 */
void build_reg_array_app_path (HKEY top_key)
{
  HKEY   key = NULL;
  int    num;
  REGSAM acc = reg_read_access();
 ...
 */
void scan_reg_environment (HKEY top_key, const char *sub_key,
                 ...
                 bool is_dir, bool exp_ok, const char *prefix, HKEY key)
{
  ...
  if (!ignore && check_empty && is_dir && dir_is_empty(_path))
...
static void init_all (const char *argv0)
{
  ...
  where_am_I = STRDUP (who_am_I);
  char *p = strrchr (where_am_I, '\\');
  *p = '\0';
