...
static int findfirst (const char *file_spec, struct ffblk *ffblk, int attrib)
{
  ...
  strncpy (ffblk->ff_name, ff_data.cFileName, sizeof(ffblk->ff_name));
  ARGSUSED (attrib);
  return (0);
}
                      ...
                      int first)  /* rest is ptr to null or ptr after slash, bp after slash */
{
  ...
  DEBUGF (("glob_dirs[%d]: rest=`%s' %c epathbuf=`%s' %c pathbuf=`%s'\n",
...
static int glob2 (const char *pattern, char *epathbuf)  /* both point *after* the slash */
{
  ...
  *bp = 0;
  ...
  DEBUGF (("glob2(): initial segment is `%s', wildcard_nesting: %d\n",
  ...
  while (!done)
  {
        ...
        (strcmp(ff.ff_name, ".") && strcmp(ff.ff_name, "..")))
    {
                  ...
                  FNM_FLAG_NOCASE) == FNM_MATCH)
      {
        ...
        {
          DEBUGF (("ffmatch: `%s' matching `%s', add `%s'\n",
...
int main (int argc, char **argv)
{
  ...
  rc = glob (argv[1], GLOB_NOSORT | GLOB_MARK, NULL, &res);
  if (rc != 0)
